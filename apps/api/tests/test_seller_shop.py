...
async def test_owner_reads_and_updates_shop(client, seller_factory) -> None:
    ...
    read = await client.get("/api/v1/seller/shop", cookies=ctx["cookies"])
...
async def test_manager_cannot_update_shop(client, seller_factory) -> None:
    ...
    response = await client.patch(
        "/api/v1/seller/shop", json={"name": "Nope"}, cookies=ctx["cookies"]
    )
...
async def test_owner_adds_and_lists_members(client, seller_factory) -> None:
    ...
    members = await client.get("/api/v1/seller/shop/members", cookies=ctx["cookies"])
...
async def test_owner_cannot_remove_self(client, seller_factory, db) -> None:
    ...
    response = await client.delete(
        f"/api/v1/seller/shop/members/{own_id}", cookies=ctx["cookies"]
    )
...
async def test_cross_tenant_member_access_blocked(client, seller_factory) -> None:
    ...
    b_members = await client.get("/api/v1/seller/shop/members", cookies=ctx_b["cookies"])
    ...
    deleted = await client.delete(
        f"/api/v1/seller/shop/members/{b_member_id}", cookies=ctx_a["cookies"]
    )
